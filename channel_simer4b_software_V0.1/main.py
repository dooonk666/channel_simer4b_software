...
# CLK_TS=1000/60  # 1/60 us = 16.667 ns
CLK_TS = 1000/120
FREQ_UNIT=1/21.47483648 # 非信号源控制
SS_FREQ_UNIT=1/35.791394133 #内置信号源频率单位
...
def make_send_DDR_ctrl():   ## 输入时延
    ...
    DDR_delay=float(DDR_delay)
    DDR_delay=round(DDR_delay/CLK_TS)
...
def apply_json_config():
    ...
    try:
        ...
        for path_idx, cluster in enumerate(clusters[:num_clusters]):
            ...
            # 转换delay：从秒转换为时钟周期数 (CLK_TS = 1/60us = 16.667ns)
